...
for frame, donor_ix, *_ in hbonds.results.hbonds:
    ...
    idx = int((frame-trj_skip)/trj_step)
    HB_counts_list[int((frame-trj_skip)/trj_step)] += hist * 2
...
HBfile = open(os.path.join(save_path, "HBperH2O_step{0}_dr{1}_angle{2}.txt".format(trj_step,delta_r,angle)), 'w+')
...
def HB_fig(bin_centers,counts_avg,counts_up,counts_down,trj_step,delta_r,save_path,angle):
    ...
    fig.savefig(os.path.join(save_path, "HBperH2O_step{0}_dr{1}_angle{2}.png".format(trj_step,delta_r,angle)), dpi=600, bbox_inches='tight')    

HB_fig(bin_centers,counts_avg,counts_up,counts_down,trj_step,delta_r,save_path,angle)