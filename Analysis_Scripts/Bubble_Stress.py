...
delta_r     = 2 # bin slice (Angstrom)
trj_step    = 1
trj_skip    = 0
####################change above####################
Pfile = open(os.path.join(save_path, "Press_step{0}_dr{1}.txt".format(trj_step,delta_r)), 'w+')
...
choose_frames = range(trj_skip,len(u_x.trajectory),trj_step)
...
def press_fig(save_path,stress_dict,virial_dict,kineti_dict,trj_step,delta_r):
    ...
    #fig.show()
    fig.savefig(os.path.join(save_path, "Press_step{0}_dr{1}.png".format(trj_step,delta_r)), dpi=600, bbox_inches='tight')    

press_fig(save_path,stress_dict,virial_dict,kineti_dict,trj_step,delta_r)